...
class EventDrawProperties {
  ...
  EventDrawProperties(DayView dayView, UICalendarItem event, this.isRTL) {
  ...
  Widget createWidget(BuildContext context, DayView dayView, Widget? resizeGestureDetector, UICalendarItem event) {
    ...
    // TODO(Stas): у меню отключен ресайз, если фича растягивания нужна то включить и переделать
    if (resizeGestureDetector != null && event.calendarItemType != CalendarItemType.popupMenu) {
      child = Stack(
        children: [
  ...
  Widget _getDraggableOrLongPressDraggable({
    ...
    if (isLongPress) {
      return LongPressDraggable<UICalendarItem>(
        data: data,
        axis: axis,
        feedback: feedback,
