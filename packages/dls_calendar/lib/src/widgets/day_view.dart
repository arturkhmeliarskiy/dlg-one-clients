...
import 'package:dls_calendar/src/controller/day_view.dart';
...
import 'package:dls_calendar/src/utils/callback_helpers.dart';
import 'package:dls_calendar/src/utils/event_grid.dart';
...
class _DayViewState extends ZoomableHeadersWidgetState<DayView> {
  ...
  final Map<UICalendarItem, EventDrawProperties> eventsDrawProperties =
      HashMap();

  /// The flutter week view events.
  late List<UICalendarItem> events;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.dragAndDropOptions == null) {
      ...
      mainWidget = DragTarget<UICalendarItem>(
  ...
  Widget createMainWidget() {
    ...
    if (widget.onBackgroundHover != null) {
      children.add(
        Positioned.fill(
          child: MouseRegion(
            onHover: (event) {
              DateTime timeTapped = widget.date.add(
                  calculateOffsetHourMinute(event.localPosition).asDuration);
              widget.onBackgroundHover!(timeTapped);
            },
            // onExit: (pointerExitEvent) {
            //   if (widget.onBackgroundHoverStop != null) {
            //     widget.onBackgroundHoverStop!();
            //   }
            // },
            child: Container(
  ...
  Widget? buildResizeGestureDetector(UICalendarItem event) {
  ...
  void onEventResizeUpdate(UICalendarItem event, double resizeOffset) {
  ...
  Widget createBackground() => Positioned.fill(
        child: CustomPaint(
          painter: widget.style.createBackgroundPainter(
              ...
              hoursColumnStyle: widget.hoursColumnStyle),
        ),
      );
  ...
  void createEventsDrawProperties() {
    ...
    for (UICalendarItem event in List.of(events)) {
