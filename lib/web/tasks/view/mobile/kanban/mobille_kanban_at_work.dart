...
class MobileKanbanAtWork extends StatefulWidget {
  const MobileKanbanAtWork({
  ...
  @override
  State<MobileKanbanAtWork> createState() => _MobileKanbanAtWorkState();
}

class _MobileKanbanAtWorkState extends State<MobileKanbanAtWork> {
  ...
  @override
  void didUpdateWidget(covariant MobileKanbanAtWork oldWidget) {
    _isActiveTaskAtWork = widget.isActiveTaskAtWork;
    _indexTaskAtWork = widget.indexTaskAtWork;
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Text(
              S.current.at_work(widget.quantity),
            ...
            builder: (context, state) {
              return Column(
                children: List.generate(
                  state.model.listTaskAtWork.length,
                  (index) {
                    return ListView(
                      shrinkWrap: true,
                      reverse: _indexTaskAtWork <= 1,
                      children: [
                        if (state.model.listTaskAtWork[index].roles?.first
                                .name !=
                            S.current.observer)
                          MobileKanbanItemTasks(
                            index: index,
                            task: state.model.listTaskAtWork[index],
                            openTask: () async {
