import 'package:dls_one/core/core.dart';
import 'package:dls_one/web/notifications/notifications.dart';
import 'package:flutter/material.dart';
import 'package:notifications_models/notifications_models.dart';

/// изменили аватар группы
class DLSNotificationChatChangeGroupAvatarWidget extends StatelessWidget {
  const DLSNotificationChatChangeGroupAvatarWidget({
    required this.object,
    required this.user,
    required this.avatarPrev,
    required this.avatarCur,
    this.isShadowEnabled = true,
    this.actions,
    this.contentCallbacks,
    super.key,
  });

  /// data
  final DLSNotificationChat object;
  final DLSNotificationUser user;
  final String avatarPrev;
  final String avatarCur;

  /// widget params
  final bool isShadowEnabled;
  final Widget? actions;
  final ContentCallbacks? contentCallbacks;

  @override
  Widget build(BuildContext context) {
    return DLSNotificationUserParentWidget(
      onTap: contentCallbacks?.onTap,
      username: user.username,
      userAvatar: user.avatar,
      subAvatarType: DLSNotificationSubAvatarType.pen,
      timestamp: object.timestamp,
      dataLevel1: S.current.chatChangeGroupAvatar,
      dataLevel2: object.name,
      isShadowEnabled: isShadowEnabled,
      actions: actions,
      child: DLSNotificationChangesTypeDLSImage(
        versionPrev: avatarPrev,
        versionCur: avatarCur,
      ),
    );
  }
}
